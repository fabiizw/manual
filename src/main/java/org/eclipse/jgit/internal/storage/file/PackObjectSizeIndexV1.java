...
class PackObjectSizeIndexV1 implements PackObjectSizeIndex {
    ...
    private PackObjectSizeIndexV1(InputStream stream, int threshold,
        ...
        while ((positionEncoding = helper.readByte(stream)) != 0) {
            if ((positionEncoding & 0xFF) == (BITS_24 & 0xFF)) {
                int sz = helper.readInt(stream);
                pos24 = new UInt24Array(readNBytes(stream, sz * 3));
            } else if ((positionEncoding & 0xFF) == (BITS_32 & 0xFF)) {
                int sz = helper.readInt(stream);
    ...
    /**
     * Java 8 compatible replacement for InputStream.readNBytes
     * 
    ...
    /**
     * Reads requested number of bytes.
     *
     * @param in the input stream to read from
    ...
    private static void readFully(InputStream in, byte[] b, int off, int len) throws IOException {
        ...
        if (n < len) {
            throw new IOException("Premature EOF: expected " + len + " bytes, got " + n);
        }
    }
    ...
    private static class IntArray {
        ...
        static IntArray from(InputStream in, int ints) throws IOException {
            ...
            byte[] data = readNBytes(in, expectedBytes);
    ...
    private static class LongArray {
        ...
        static LongArray from(InputStream in, int longs) throws IOException {
            byte[] data = readNBytes(in, longs * LONG_SIZE);
